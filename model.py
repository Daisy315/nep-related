...
def read_model(filename: str) -> Model:
    ...
    for count in range(n_count):
        for s in keys:
            ...
            # need to offset n1 by +1 to handle bias                              # only +1 for two species
            n1 += count
            ...
            n1 += n_network_params
        if count == 0:
            pars['b1'] = data['ann_parameters'][n1]  #第1021个是b1
        else:
            pars['b1_polar'] = data['ann_parameters'][n1-1]  #第2042个是b1_polar,因为是多元素，并且mode=2，第3，4次循环n1都加了count
